...
public class CompilerF0xC {
	...
	public static void createOBJ() {
		File fi = INCLUDER.inTemp("output.o");
		fi.delete();
	...
	static ArrayList<String> classesToVisit = new ArrayList<String>();
	...
	public static void compile(File input) throws Exception {
        ...
        for(int index=0;index<classesToVisit.size();index++) {
        	String str = classesToVisit.get(index);
        	visitClass(str);
        }
...
//            			visitClass(insn.owner);
            			...
            			try {
            				Class<?> clas = Class.forName(insn.owner.replace('/', '.'));
            				if(clas == String.class) {
            					initString(insn);
            				}else {
            					classesToVisit.add(insn.owner);
            				}
        				}catch(Exception e) {
        					
        				}
            			System.err.println("WARNING(2): Skipping init of "+insn.owner);
            	...
            	JumpInsnNode insn = (JumpInsnNode) ins;
            	  switch(insn.getOpcode())
                  {
	                  	...
	                  	case Opcodes.IFNONNULL:	jumpNull("notnull", insn.label); break;
	                  	case Opcodes.IFNULL:	jumpNull("null", insn.label); break;
...
//            	lastAccecedVariable = insn.name;
            	...
            	//load from array
            		...
            		loadFromArray(10);
            	//load from array
            	else if(op == Opcodes.CALOAD)
            		loadFromArray(5);
            	else if(op == Opcodes.AALOAD)
            		loadFromArray(10);
            	else if(op == Opcodes.BALOAD)
            		loadFromArray(8);
            	//load from array
//            	else if(op == Opcodes.BALOAD)
//            		loadFromArray();
...
//        		System.out.println(((LocalVariableNode)method.localVariables.get(i)).desc);
        		numLocales = numLocales + 1;
        	}
//        	System.out.println(method.maxStack);
    		Integer[] stack = new Integer[numLocales];
     		LabelNode insn = (LabelNode)method.instructions.get(0);
    		System.out.println("Created stack for "+insn.getLabel()+".. num locales: "+numLocales);
	...
	private static void handleAL() {
		curPseudo.alength();
		if(DEBUG)
			System.out.println(p+"getting length");	
	}	
	...
	private static void jumpNull(String s, LabelNode label) {
		if(DEBUG)
			System.out.println(p+s+"(null): "+label.getLabel());
		curPseudo.jumpNull(s,label);
	}
	...
	public static void initString(MethodInsnNode insn) {
		curPseudo.initString(insn);
		//if(true)
		//	throw new Error("Don't know owner of method: "+insn.owner+", "+insn.name+": "+insn.desc);
		if(DEBUG)
			System.out.println(p+"Init String "+insn.name);
	}
	...
	public static void retArray(MethodNode n) {
		if(DEBUG)
			System.out.println(p+"return array");
		curPseudo.retArray(n);
	}
	...
	public static void andI() {
		if(DEBUG)
		System.out.println(p+"and integer");
		curPseudo.andInt();
	}
	public static void xorI() {
		if(DEBUG)
		System.out.println(p+"xor integer");
		curPseudo.xorInt();
	}
	...
	public static void storeByte(int index, MethodNode methodNode) {
		if(DEBUG)
    	System.out.println(p+"save index "+index+" from method "+methodNode.name);
    	curPseudo.storeTo(index, methodNode);
	}
