...
public class Pseudo {
	...
	public void storeToArray(int c) {
		write("sta "+c); //STore Array
	}
	public void loadFromArray(int c) {
		write("loa "+c); //LOad Array
	}
	...
	public void storeByte(int index, MethodNode methodNode) {
		write("bstore "+index+" "+CompilerF0xC.getLabel(methodNode));
	}
	...
	public void retArray(MethodNode methodNode) {
		write("istore 0 "+CompilerF0xC.getLabel(methodNode));
		write("retArray "+CompilerF0xC.getLabel(methodNode));
	}
	...
	public void jumpNull(String s, LabelNode label) {
		if(s.equalsIgnoreCase("null"))
			write("jmpnull "+s+" "+label.getLabel());
		else if(s.equalsIgnoreCase("notnull"))
			write("jmpnotnull "+s+" "+label.getLabel());
	}
	public void createObject(String desc) {
		write("new "+desc);
	}
	public void initString(MethodInsnNode m ) {
//		write("str_init ");
		Type[] params = Type.getArgumentTypes(m.desc);
		if(params.length == 1 && params[0].getClassName().equalsIgnoreCase("char[]"))
			write("str_chars");
		else 
			throw new Error("Don't know String decriptor "+m.desc);
	}
